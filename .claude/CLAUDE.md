...
Cache name is `radar-v49`. Two-page architecture since the landing was added:
- `/app.html` or `/app` → serves cached `./app.html`
- All other paths → tries cached `./`, falls back to cached `./app.html`
- `./` (landing page root) is NOT pre-cached to avoid storing stale 404s

Auth callback URLs (`?access_token=`, `?lastfm_token=`, etc.) are stripped from navigate requests by serving the shell from cache — the URL stays in the address bar so `handleCallback()` can read it. Bump the cache version in `sw.js` after deploying changes that must bust the cache.
